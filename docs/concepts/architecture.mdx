...
description: 'Understand the high-level architecture of PiCrust and how components work together'
---
...
```mermaid
flowchart TB
    App["Your Application<br/>(Tauri Frontend, CLI, Web Server)<br/>Chat UI / Permission dialogs / Session management"]

    App --> Runtime

    Runtime["AgentRuntime<br/>Spawns agent tasks<br/>Maintains agent registry<br/>Shares global permissions<br/>Thread-safe, cloneable"]

    Runtime --> Handle

    Handle["AgentHandle<br/>send_input / subscribe<br/>send_permission_response<br/>state / interrupt / shutdown<br/>Cloneable, shareable across threads"]

    Handle --> Agent

    subgraph Agent["StandardAgent"]
        direction TB
        Loop["Agent Loop:<br/>1. Receive input from handle<br/>2. Inject context<br/>3. Call LLM (with prompt caching)<br/>4. Parse response<br/>5. Execute tools (with permissions and hooks)<br/>6. Send output chunks to subscribers<br/>7. Save to session history<br/>8. Repeat until done"]
        subgraph Components
            direction LR
            Tools["Tools<br/>Registry"]
            Perms["Permissions<br/>Manager"]
            Hooks["Hooks<br/>Registry"]
            Session["Session<br/>(Disk)"]
            LLM["LLM<br/>Provider"]
            Context["Context<br/>Injection"]
            Debug["Debugger<br/>(Optional)"]
        end
    end
```
...
flowchart TB
    ...
    Handle["AgentHandle"]
    Agent["StandardAgent"]
    Steps["1. Process attachments<br/>2. Add to history<br/>3. Inject context<br/>4. Call LLM"]
...
flowchart TB
    ...
    S1["Subscriber 1<br/>(Frontend)"]
    S2["Subscriber 2<br/>(Logger)"]
    SN["Subscriber N"]
