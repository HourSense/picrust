...
```mermaid
flowchart TB
    S["Session Rules<br/>Highest priority, in-memory only"]
    L["Local Rules<br/>Agent-type specific, persisted to disk"]
    G["Global Rules<br/>All agents, persisted to disk"]
    U["Ask User<br/>(if interactive)"]
