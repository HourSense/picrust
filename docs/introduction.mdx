...
Unlike simple chatbot libraries, PiCrust is designed for **real applications**:
...
```mermaid
flowchart TB
    App["Your Application<br/>(Tauri, CLI, Web Server, etc.)"]
    Runtime["AgentRuntime<br/>Spawns and manages agent tasks<br/>Maintains registry of running agents<br/>Shares global permissions across agents"]
    Handle["AgentHandle<br/>External interface for communication<br/>send_input, subscribe, state<br/>Cloneable, shareable across threads"]
    Agent["StandardAgent<br/>Agent Loop: Receive - Inject Context - Call LLM -<br/>Parse Response - Execute Tools - Send Output - Repeat"]
...
PiCrust is built on these principles:
...
- **GitHub**: [PiCrust](https://github.com/yourusername/picrust)
