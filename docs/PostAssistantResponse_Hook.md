...
Assistant message added to history
  ↓
🔥 PostAssistantResponse HOOK FIRES HERE 🔥
  ↓
...
User: "Help me analyze these files"
  → LLM response with 5 tool calls
  → 5 tools execute
  → Assistant message saved to history
  → [HOOK #1 FIRES] - sees 5 ToolUse blocks

  → 5 tool results added to history
  → LLM response with 3 tool calls
  → 3 tools execute
  → Assistant message saved to history
  → [HOOK #2 FIRES] - sees 3 ToolUse blocks

  → 3 tool results added to history
  → LLM response with final text (no tools)
  → Assistant message saved to history
  → [HOOK #3 FIRES] - sees text content
  → Turn complete
```
