...
- **Thinking / extended reasoning**: `ThinkingConfig` is silently ignored — OpenAI o-series reasoning is handled by the model automatically and is not configurable via this API.
...
Supported by AnthropicProvider and GeminiProvider:
...
**OpenAIProvider**: `ThinkingConfig` is ignored. o-series models (o3, o4-mini, etc.) reason automatically — there is no token budget to configure via the Responses API.

The conversion is automatic based on the model name, so you use the same API for Anthropic and Gemini.
