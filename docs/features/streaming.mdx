...
PiCrust uses a **dual-channel** approach for conversation data:
...
flowchart LR
    Output["Agent Output"]
    Stream["Stream Channel<br/>(ephemeral)"]
    Disk["Disk Storage<br/>(durable)"]
    Subs["Subscribers<br/>(UI, Logger, etc.)"]
